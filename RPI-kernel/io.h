void mmio_write(long reg, unsigned int val);
unsigned int mmio_read(long reg);
void uart_init();
//void uart_sendc_nocheck(unsigned char c);
void uart_sendc(unsigned char c);
unsigned char uart_recvc();
void uart_sends(const char* str);
enum {
  PERIPHERAL_BASE = 0x3F000000,
  GPIO_BASE = PERIPHERAL_BASE + 0x200000,
  GPFSEL0 = GPIO_BASE,
  GPFSEL1 = GPIO_BASE + 0x04,
  GPSET0 = GPIO_BASE + 0x1C,
  GPCLR0 = GPIO_BASE + 0x28,
  GPPUD       = GPIO_BASE + 0x94,
  GPPUDCLK0   = GPIO_BASE + 0x98
};
enum{
  UART0_BASE = 0x3F201000,
  UART0_DR     = (UART0_BASE + 0x00),
  UART0_RSRECR = (UART0_BASE + 0x04),
  UART0_FR     = (UART0_BASE + 0x18),
  UART0_ILPR   = (UART0_BASE + 0x20),
  UART0_IBRD   = (UART0_BASE + 0x24),
  UART0_FBRD   = (UART0_BASE + 0x28),
  UART0_LCRH   = (UART0_BASE + 0x2C),
  UART0_CR     = (UART0_BASE + 0x30),
  UART0_IFLS   = (UART0_BASE + 0x34),
  UART0_IMSC   = (UART0_BASE + 0x38),
  UART0_RIS    = (UART0_BASE + 0x3C),
  UART0_MIS    = (UART0_BASE + 0x40),
  UART0_ICR    = (UART0_BASE + 0x44),
  UART0_DMACR  = (UART0_BASE + 0x48),
  UART0_ITCR   = (UART0_BASE + 0x80),
  UART0_ITIP   = (UART0_BASE + 0x84),
  UART0_ITOP   = (UART0_BASE + 0x88),
  UART0_TDR    = (UART0_BASE + 0x8C)
};
enum{
  MAIL_BASE = 0xB880,
  MAIL_READ = MAIL_BASE+0x00,
  MAIL_POLL = MAIL_BASE+0x10,
  MAIL_SENDER = MAIL_BASE+0x14,
  MAIL_STATUS = MAIL_BASE+0x18,
  MAIL_CONFIG = MAIL_BASE+0x1c,
  MAIL_WRITE = MAIL_BASE+0x20,
  MAIL_FULL = 0x80000000,
  MAIL_EMPTY = 0x40000000,
  MAIL_RESPONSE = 0x80000000
};
